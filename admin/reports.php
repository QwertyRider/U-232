...
if ((isset($_GET["deal_with_report"])) || (isset($_POST["deal_with_report"]))) { 
    ...
    mysql_query ("UPDATE reports SET delt_with = 1, $how_delt_with, $when_delt_with , who_delt_with_it = $CURUSER[id] WHERE delt_with!=1 AND id = $_POST[id]") or sqlerr(__FILE__, __LINE__);
    }
    ...
    $res = mysql_query("DELETE FROM reports WHERE id = $id") or sqlerr(__FILE__, __LINE__);
    $HTMLOUT .="<h1>Report Deleted!</h1>\n";
    }
    // === get the count make the page
   $res = mysql_query("SELECT count(id) FROM reports") or sqlerr(__FILE__, __LINE__);
   ...
   " . ($CURUSER["class"] == UC_SYSOP ? "<td class='colhead' align='center' valign='top'>Delete</td>" : "") . "</tr>";
    ...
    while ($arr_info = mysql_fetch_assoc($res_info)) {
        ...
        <td align='center' valign='middle' class='$class'>{$checkbox}</td>" . ($CURUSER["class"] == UC_SYSOP ? "<td align='center' valign='middle' class='$class'><a class='altlink' href='admin.php?action=reports&amp;id=" . $arr_info['id'] . "&amp;delete=1'><font color='red'>Delete</font></a></td>" : "") . "</tr>\n";
