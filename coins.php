...
/************************************************
*
*  System under construction - author neglected to post working nor full code ...
*
*
****************************************************/
...
$prizes=array($FREE,$FREE2,$FREE3,$FREE4);
  ...
  function kaupa($PRI){
    ...
    if(isset($_GET['kaupa'])){
	  ...
	  if($id == 1){
		...
		kaupa($FREE['prize']/2);
		else kaupa($FREE['prize']);
		$free_switch = (1 * 86400 + time());
		if ($CURUSER['free_switch'] != 0)
    stderr("Error", "Time shall unfold what plighted cunning hides\n\nWho cover faults, at last shame them derides.");
		sql_query("UPDATE users SET free_switch=$frees_switch WHERE id=$CURUSER[id]") or sqlerr(__FILE__,__LINE__);
	...
	foreach($prizes as $P){
		if($CURUSER["class"] >= UC_VIP)$VERD=($P['prize']/2);
		else $VERD=$P['prize'];
		$HTMLOUT.="<tr>
			<td>$VERD</td>
			<td>$P[name]</td>
			<td>$P[desc]</td>
			<td><a href='{$TBDEV['baseurl']}/coins.php?kaupa=$P[id]'><input type='submit' value='{$lang['pr_kaupa']}' /></a></td>
			</tr>";
	}

$HTMLOUT.="</table>";
print stdhead($lang['head_verslun']) . $HTMLOUT . stdfoot();
?>