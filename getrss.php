...
HTML;
	...
	while($a = mysql_fetch_assoc($q1)) {
		if($i%5 == 0 && $i>0)
			$HTMLOUT .="<br/>";
		$HTMLOUT .= "<label for=\"cat_{$a['id']}\"><img src=\"{$TBDEV['pic_base_url']}caticons/{$a['image']}\" alt=\"{$a['name']}\" title=\"{$a['name']}\" /><input type=\"checkbox\" name=\"cats[]\" id=\"cat_{$a['id']}\" value=\"{$a['id']}\" /></label>\n";
		$i++;
	}
$HTMLOUT .= <<<HTML
</td>
