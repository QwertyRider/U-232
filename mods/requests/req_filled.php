...
$res = mysql_query("SELECT id FROM torrents WHERE id = $torrentid") or sqlerr(__FILE__,__LINE__);
$arr = mysql_fetch_assoc($res);

if (!$arr)
    stderr("{$lang['error_error']}", "{$lang['error_no_torrent2']}$torrentid");
          
$res = mysql_query("SELECT users.username, requests.userid, requests.torrentid, requests.request FROM requests inner join users on requests.userid = users.id where requests.id = $id") or sqlerr(__FILE__, __LINE__);
$arr = mysql_fetch_assoc($res);
...
if ($TBDEV['karma'] && isset($CURUSER['seedbonus']))
    mysql_query("UPDATE users SET seedbonus = seedbonus+".$TBDEV['req_comment_bonus']." WHERE id = $CURUSER[id]") or sqlerr(__FILE__, __LINE__);

$res = mysql_query("SELECT `userid` FROM `voted_requests` WHERE `requestid` = $id AND userid != $arr[userid]") or sqlerr(__FILE__, __LINE__);
