 ...
 **/
$res = mysql_query("SELECT userid, cat FROM requests WHERE id = $id") or sqlerr(__FILE__, __LINE__);
...
if ($filled) {
  ...
  //  }    
   $res = mysql_query("SELECT id FROM torrents WHERE id = ".$torrentid);
    if (mysql_num_rows($res) == 0)
           stderr("{$lang['error_error']}", "{$lang['error_no_torrent']}");
    	   
    mysql_query("UPDATE requests SET cat = $cat, request = $request, descr = $descr, filledby = $filledby, torrentid=$torrentid WHERE id = $id") or sqlerr(__FILE__,__LINE__);
}
else
    mysql_query("UPDATE requests SET cat = $cat, filledby = 0, request = $request, descr = $descr, torrentid = 0 WHERE id = $id") or sqlerr(__FILE__,__LINE__);
